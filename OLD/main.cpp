...
private:
  ...
  {
    if (Button == GLUT_RIGHT_BUTTON && State == GLUT_DOWN && !IsPrimitiveStarted)
    {
    ...
    else if (IsPrimitiveStarted && !IsSegmentStarted && Button == GLUT_RIGHT_BUTTON && State == GLUT_DOWN)
    {
      SegmentPreview.first = CurrPolyLastLineEnd();
      FindPoint(CurrPolyLastLineEnd(), TRUE, &SegmentPreview.first);
      IsSegmentStarted = TRUE;
    }
   
    if (Button == GLUT_RIGHT_BUTTON && State == GLUT_UP && IsSegmentStarted && IsPerpendicular)
    {
      SegmentPreview.second = CurrPolyGetPerpPoint(GetScreenCoord(X, Y));
    ...
    else if (Button == GLUT_RIGHT_BUTTON && State == GLUT_UP && IsSegmentStarted)
    {
      SegmentPreview.second = GetScreenCoord(X, Y);
      IsPrimitiveStarted = !CurrPolyPlaceSegment(SegmentPreview.second, SegmentPreview.first);
      IsSegmentStarted = FALSE;
  ...
  VOID MouseMotion( INT X, INT Y ) override final
  {
    ...
    {
      vec2 p = GetScreenCoord(X, Y);

      if (!IsPerpendicular)
        SegmentPreview.second = p;
      else
        SegmentPreview.second = CurrPolyGetPerpPoint(p);
      FindPoint(SegmentPreview.second, TRUE, &SegmentPreview.second);
    }

    if (IsMiddleButton)
    {
      vec2 p2m = GetScreenCoord(X, Y);

      CurrPolyMovePoint(p2m, TRUE);
    }
