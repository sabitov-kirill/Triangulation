...
private:
  ...
  const DBL PlaceingRadius = 0.04; // Points  for segments placment radius
...
public:
  ...
  {
    size_t c = CurrPoly.Lines.size();

    if (c <= 0)
      return -1;
    return CurrPoly.Lines[c - 1].End;
  ...
  {
    size_t Ind = CurrPolyLastLineEndIndex();
  
    return PointsPool[Ind];
  ...
  VOID CurrPolyMovePoint(vec2& Point, BOOL flag);
